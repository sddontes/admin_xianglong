      ...
      <el-row>
        ...
        <el-table :data="repayOrderList" v-loading="repayOrderListLoading">
          ...
          <el-table-column prop="repayType" label="还款方式">
            <template scope="scope">{{formatRepayType(scope.row.repayType)}}</template>
          </el-table-column>
        </el-table>
      </el-row>
  ...
  export default {
    ...
    methods: {
      ...
      orderDataDetail(){
          ...
          if (1 == body.code) {
            if(body.data){
              ...
              if(body.data.status == 7){    //查询订单拒绝原因
                ...
                this.refuseReasonDetail = body.data.remark;
                this.refuseReason = body.data.remark;
              ...
              if(body.data.channel == 11 || body.data.channel == 50 || body.data.channel == 110){                  //芝麻分
        ...
        /*催收标签*/
        this.$http.post('manage/orderTagInfo',{orderNo:this.uuid}).then(response => {
          let {body} = response;
          if(1 == body.code && body.data){
        ...
        this.$http.post('manage/userBaseInfo', { uuid:this.userUuid}).then(response => {
          if (1 == response.body.code) {
            if(response.body.data && response.body.data[0]){
              ...
              // this.addreBook(this.customerInfo.channel);        //通讯录信息兼容360渠道
              // this.getTop20ContactList(this.customerInfo.channel);  
              this.getIsCompanyAll(this.customerInfo.channel);  //通话记录兼容360
            }
          }
        });
       ...
       */
      thirdDataInfo(){
        this.getUserThirdData({uuid:this.userUuid,dataType:3},(data) =>{    //水印VS活体相似度
          try{
            let result = JSON.parse(data.data);
       ...
       * 判断是否认证完成
       */
       getIsCompanyAll(channel){
        this.$http.post("manage/completionInfoByUserUuid",{userUuid:this.userUuid}).then((response) =>{
          let {body} = response;
          if(1 == body.code){
            if(body.data){
              this.addreBook(channel,body.data["completionAll"]);        //通讯录信息兼容360渠道
              this.getTop20ContactList(channel,body.data["completionAll"])
            }else {
              this.addreBook(channel,0); 
              this.getTop20ContactList(channel,0)
            }
          }
        })
       },
      ...
      addreBook(channel,e){
        ...
        let _url = '';
        let _data = {};
        (channel == 11|| channel == 50|| channel == 110) ? _url = 'manage/rong360UserContactByUserUuidMongo' : _url = 'manage/orderUserContactMongoByOrderNo';
        (channel == 11|| channel == 50|| channel == 110) ? _data = {uuid: this.uuid,userUuid:this.userUuid,channel:channel} : _data = {uuid: this.uuid,userUuid:this.userUuid};
        if(e){
          _url = 'manage/orderUserContactMongoByOrderNo';
        }
