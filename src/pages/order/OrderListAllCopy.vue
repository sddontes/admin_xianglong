...
<template>
  ...
  <!--       <el-table-column label="手机号" prop="mobileNumber" width="130">
        ...
        <el-table-column label="操作" width="280" prop="status">
          <template scope="scope">
            <el-button
              type="primary"
              size="small"
            ...
            <el-button
              ...
              type="primary"
              size="small"
              @click="passCheck(scope.$index, scope.row)">手动放款
            </el-button>            
<!--             <el-button
              v-if='scope.row.status==2?"true":false'
              type="primary"
              size="small"
              @click="passSysCheck(scope.$index, scope.row)">申请代付
            </el-button> -->
  ...
  export default {
    ...
    methods: {
      ...
      passCheck(index,row) {
        ...
        this.$confirm('此操作将通过审核进行放款, 是否继续?', '提示', {
          ...
          if(row.status!=2){
            ...
            this.$http.post('manage/updateOrderToLoaned', {"orderNo":row.uuid}).then(response => {
              if (1 == response.body.code) {
                this.$message({
                  message: '放款成功',
                  type: 'success'
                });
                this.gridData[index].status=5;
              } else {
                this.$message.error(response.body.message);
              }
            }, response => {
      ...
      passSysCheck(index,row) {
        ...
        this.$confirm('此操作将通过审核进行放款, 是否继续?', '提示', {
          ...
          if(row.status!=2){
            ...
            this.$http.post('manage/systemToLoaned', {"orderNo":row.uuid}).then(response => {
              if (1 == response.body.code) {
                this.$message({
                  message: '放款成功',
                  type: 'success'
